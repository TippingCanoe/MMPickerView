...
@property (copy) void (^selectionBlock)(id selectedObject);
@property (copy) void (^completionBlock)(id selectedObject);
@property (copy) void (^cancellationBlock)(id selectedObject);
...
+ (void)showPickerViewInView:(UIView *)view
                 ...
                 withOptions:(NSDictionary *)options
                   selection:(void (^)(id))selection
                  completion:(void (^)(id))completion
                cancellation:(void (^)(id))cancellation{
    ...
    [self sharedView].selectionBlock = selection;
    [self sharedView].completionBlock = completion;
    [self sharedView].cancellationBlock = cancellation;
...
- (void)dismiss:(BOOL)cancel{
    [MMPickerView dismissWithCompletion:cancel ? self.cancellationBlock : self.completionBlock];
}
...
- (void)initializePickerViewInView: (UIView *)view
    ...
    _pickerViewDoneBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(done)];
    _pickerViewCancelBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancel)];
    
    if (([[[UIDevice currentDevice] systemVersion] compare:@"7.0" options:NSNumericSearch] == NSOrderedAscending)) {
        [_pickerViewCancelBarButtonItem setTintColor:[UIColor blackColor]];
    }
    
    _pickerViewToolBar.items = @[_pickerViewCancelBarButtonItem, flexibleSpace, _pickerViewDoneBarButtonItem];
    [_pickerViewDoneBarButtonItem setTintColor:buttonTextColor];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.selectionBlock) {
        self.selectionBlock([_pickerViewArray objectAtIndex:row]);
    }
}
